...
const EditHabitModal = ({
  ...
  const [name, setName] = useState<string>(editHabit.name);
  const [goal, setGoal] = useState<number | "">(editHabit.goal);
  const handleSave = () => {
    if (!name.trim() || goal === undefined || goal === null || goal === "")
      return;
    handleEditHabit({ ...editHabit, name: name.trim(), goal: Number(goal) });
    setName("");
    setGoal("");
    onClose();
  };
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.title_btn}>
          <div className={styles.title_text}>Edit Habit</div>
